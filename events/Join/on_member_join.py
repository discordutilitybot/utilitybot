    ...
    async def on_member_join(self, member):
        ...
        if logch:
            embed = discord.Embed(title="Member Joined",
                                  url="https://tenor.com/view/penguin-hello-hi-hey-there-cutie-gif-3950966")

            embed.set_author(name=f"{member}", icon_url=str(
                member.avatar_url_as(static_format='png'), size=2048))

            embed.add_field(name='Account Created', value=str(datetime.datetime.utcnow() - member.created_at) + 'ago',
                            inline=False)
            if usedinvite:
                embed.add_field(name="Invite used:", value=used_invite, inline=False)

            embed.add_footer(name="Member Count:", value=f"{member.guild.members}")
